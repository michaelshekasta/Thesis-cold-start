...
from keras.layers import Masking, LSTM, Dense, Embedding
...
class printTest(Callback):
    def __init__(self, x_test, y_test, exp_name=""):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.calc_auc_epoch(epoch, 'regular', self.x_test, self.y_test)
            ...
            with open("predictions.csv", "a") as fw2:
                ...
                for i in range(y_pred.shape[0]):
                    ...
                    fw2.write("%s,%s,%s,%s,%s,%s,%s,%s\n" % (
                        self.exp_name, str(epoch), str(i), str(x_test[i]).replace("\n", " ").replace(",", ";"),
...
class RegularExp(object):
    ...
    def run_exp(self, x_train, y_train, x_test, y_test, use_cnn=False, shuffle=False, validation_split=False):
        self.print_states("Run Training set ", y_train)
        self.print_states("Run non new item test set", y_test)
        callbacks = [
            printTest(x_test=x_test, y_test=y_test, exp_name=self.exp_name),
        ...
        if system_utils.is_file_exist("%s/models.h5" % self.model_path):
            ...
            if self.use_class_weight:
                ...
                model.fit(x=x_train, y=y_train, epochs=self.epochs_model, class_weight=dict(enumerate(class_weights)),
        ...
        print('testing..')
        return self.evalute_model(model, x_test, y_test)
